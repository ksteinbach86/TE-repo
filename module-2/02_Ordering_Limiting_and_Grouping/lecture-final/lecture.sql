...
SELECT park_name
FROM park
ORDER BY (CURRENT_DATE - date_established) / 365 DESC, park_name ASC
LIMIT 20;
...
SELECT ('Name: ' || park_name || ', Date established: ' || date_established || ', Area: ' || area) AS description
FROM park
...
FROM state
-- WHERE census_region = 'Midwest' OR census_region = 'West'
...
SELECT COUNT(city_name) as big_cities_count
FROM city
WHERE population > 1000000;


-- The number of state nicknames.
SELECT COUNT(state_name) AS state_count, COUNT(state_nickname) AS nickname_count, COUNT(*) AS row_count
FROM state
...
SELECT AVG(area) AS avg_park_area, has_camping
FROM park
