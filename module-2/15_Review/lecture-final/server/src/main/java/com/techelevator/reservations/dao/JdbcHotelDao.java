...
public class JdbcHotelDao implements HotelDao {
    ...
    public List<Hotel> list() {
        ...
        SqlRowSet results = jdbcTemplate.queryForRowSet("SELECT id, name, address, address2, city, state, zip, stars, rooms_available, cost_per_night FROM state;");
        while (results.next()) {
    ...
    public Hotel get(int id) {
        Hotel hotel = null;
        String sql = "SELECT id, name, address, address2, city, state, zip, stars, rooms_available, cost_per_night FROM state WHERE id = ?;";
